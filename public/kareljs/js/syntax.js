CodeMirror.defineMode('karelpascal', function() {
  ...
  function tokenBase(stream, state) {
    ...
    if (keywords.propertyIsEnumerable(cur))
      ...
      style = 'builtin';
    else if (operator.propertyIsEnumerable(cur))
      style = 'operator';
    else if (atoms.propertyIsEnumerable(cur))
      style = 'atom';
    else if (indent.propertyIsEnumerable(cur))
      style = 'indent';
    else if (dedent.propertyIsEnumerable(cur))
      style = 'dedent';
  ...
  return {
    startState: function() { return {tokenize: null, lastTok: null}; },
...
CodeMirror.defineMode('kareljava', function() {
    ...
    var keywords = words(
        'program class if else iterate  while turnoff void ');
    var indent = words(
        '{');
    var dedent = words('}');
    var builtin = words(
        'pickbeeper putbeeper turnleft move turnoff');
    var operator = words('&& || ! succ pred');
    var atoms = words(
