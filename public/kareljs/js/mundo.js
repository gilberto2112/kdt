...
var WorldRender = function(context, worldHeight, worldWidth,mainDomObject) {
  ...
  this.paint = function(world, mundo_ancho, mundo_alto, options) {
    ...
    for (var fila = this.primera_fila;
      ...
      num_columna = 1;
      for (var columna = this.primera_columna;
        ...
        if ((paredes & 0x1) != 0) {  // oeste
          context.fillRect(origen.x + (num_columna - 1) * 30 - 1,
        ...
        if ((paredes & 0x4) != 0) {  // este
          context.fillRect(origen.x + (num_columna - 1) * 30 - 1 + 30,
                           origen.y - (num_fila - 1) * 30, 4, 30);
        }
        ...
        if (zumbadores == -1 || zumbadores > 0) {
          ...
          if (zumbadores == -1) {
            context.fillRect(origen.x + (num_columna - 1) * 30 + 8,
                             origen.y - (num_fila - 1) * 30 + 8, 16, 12);

            context.font = '25px monospace';
            context.fillStyle = '#000000';
            context.fillText('∞', origen.x + (num_columna - 1) * 30 + 9,
                             origen.y - (num_fila - 1) * 30 + 23);
          } else if (zumbadores < 10) {
            context.fillRect(origen.x + (num_columna - 1) * 30 + 9,
                             origen.y - (num_fila - 1) * 30 + 8, 12, 14);

            context.font = '12px monospace';
            context.fillStyle = '#000000';
            context.fillText(String(zumbadores),
                             origen.x + (num_columna - 1) * 30 + 11,
                             origen.y - (num_fila - 1) * 30 + 20);
          } else {
            context.fillRect(origen.x + (num_columna - 1) * 30 + 7,
    ...
    // Numeros de colummna
    a = 1;
    for (i = this.primera_columna; i < this.primera_columna + this.num_columnas;
         i++) {
      context.font = '14px monospace';
      context.fillStyle = '#000000';
    ...
    if (typeof document !== 'undefined') {
        $(mainDomObject.mochila).get(0).value = world.bagBuzzers;
    }
  ...
  this.hoverRuler = function(fila, columna, mundo_ancho, mundo_alto) {
    ...
    context.fillRect(6, origen.y - (fila - this.primera_fila) * 30 + 3,
  ...
  this.hoverWall = function(fila, columna, orientacion, mundo_ancho,
                            mundo_alto) {
    // Coordenada para dibujar la primera casilla
    var origen = {x: 30, y: mundo_alto - 60};
    context.fillStyle = 'rgba(255,0,0,0.5)';
